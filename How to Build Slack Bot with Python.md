title: How to Build Slack Bot with Python (based on https://www.fullstackpython.com/blog/build-first-slack-bot-python.html)

Note: since my chatbot is named as h1bpedia, therefore all of the "StarterBot" appear in the following script should be replaced with h1bpedia.
...
## Tools We Need
Our bot, which we will name "h1bpedia", requires Python and the Slack API.
...
the `pip` command. If you come across an error saying permission denied, try to rename the folders in your path with one word:
...
Name your bot "h1bpedia" then click the “Add bot integration” button.
...
Save the script under your working directory. We only need to run this script once to obtain our bot’s ID.
...
environment variable named `BOT_ID`.
    ...
    (h1bpedia)$ export BOT_ID='bot id returned by script'
...
exported as an environment variable. 
    ...
    if __name__ == "__main__":
        ...
        if slack_client.rtm_connect():
            print("h1bpedia connected and running!")
            while True:
...
Now that all of our code is in place we can run our h1bpedia chatbot on the 
command line with the `python h1bpedia.py` command.
